...
def Optimization(num_vars, bounds, whether_to_optimize, num_vars_all, Model,
                                              Training_set,
                                              y_Training_set, default_values, r,
                                              max_iteration,
    ...
    for i in range(max_iteration):
        ...
        y_new = Running.objective_function(part_x_new, )
        ...
        time_optimization.append(time.time() - time_start)

    return Pareto_front, Pareto_solution, All_PF, time_optimization
...
if __name__ == "__main__":
    ...
    Training_set = np.load('Training_set.npy')[:initial_sample_size, :]
    y_Training_set = np.load('y_Training_set.npy')[:initial_sample_size, :]

    obj = y_Training_set.shape[1]

    whether_to_optimize = np.load('whether_to_optimize.npy')
    ...
    for j in range(repetition):
        ...
        Pareto_front, Pareto_solution, All_PF, t = Optimization(num_vars, bounds, whether_to_optimize, num_vars_all, Model,
                                                  Training_set,
                                                  y_Training_set, defaults, r,
                                                  max_iteration,
                                                  time_start, repetition)
        ...
        np.save('PF-repetition-' + str(j) + '.npy', Pareto_front)
        np.save('PS-repetition-' + str(j) + '.npy', Pareto_solution)
        with open('All_PF-repetition-' + str(j) + '.pkl', 'wb') as f:
            pickle.dump(All_PF, f)

        np.save('RHD-repetition-' + str(j) + '.npy', rhd)
        np.save('Time-repetition-' + str(j) + '.npy', np.array(t))
