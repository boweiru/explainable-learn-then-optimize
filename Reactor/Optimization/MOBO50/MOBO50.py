...
if __name__ == '__main__':
    ...
    Training_set = np.load('Training_set.npy')[:initial_size, :]
    y_Training_set = np.load('y_Training_set.npy')[:initial_size, :]

    for j in range(repetition):
        rhd = []
        time_start = time.time()
        Ports = 7001 + repetition
        container_name = 'MOBO50' + str(Ports)

        Pareto_front, Pareto_solution, All_PF, t = Bayesian_optimization_Simulation_scenario(num_vars, bounds,
                                                                                             Training_set,
                                                                                             y_Training_set, defaults,
        ...
        np.save('.\\Results\\PF-MOBO50-Reactor-repetition-' + str(j) + '.npy', Pareto_front)
        np.save('.\\Results\\PS-MOBO50-Reactor-repetition-' + str(j) + '.npy', Pareto_solution)
        with open('.\\Results\\All_PF-MOBO50-Reactor-repetition-' + str(j) + '.pkl', 'wb') as f:
            pickle.dump(All_PF, f)

        np.save('.\\Results\\RHD-MOBO50-Reactor-repetition-' + str(j) + '.npy', rhd)
        np.save('.\\Results\\Time-MOBO50-Reactor-repetition-' + str(j) + '.npy', np.array(t))
