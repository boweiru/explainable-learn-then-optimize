...
class MyProblem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        out["F"] = Running.objective_function(x, self.defaults, self.whether_to_optimize, self.Ports,
                                                  self.container_name)
...
Ports = 2001 + repetition
container_name = 'NSGA2' + str(Ports)
...
for i in range(repetition):
    ...
    np.save('.\\Results\\Time-NSGA2-Reactor-repetition-' + str(i) + '.npy', time.time() - start_time)
    ...
    np.save('.\\Results\\PF-NSGA2-Reactor-repetition-' + str(i) + '.npy', Pareto_front)
    np.save('.\\Results\\PS-NSGA2-Reactor-repetition-' + str(i) + '.npy', Pareto_solution)

    np.save('.\\Results\\RHD-NSGA2-Reactor-repetition-' + str(i) + '.npy', np.array(rhd))
