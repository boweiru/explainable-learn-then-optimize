...
def Bayesian_optimization_Simulation_scenario(num_vars, bounds, whether_to_optimize, num_vars_all,
                                              Training_set,
                                              y_Training_set, default_values, Model, r, Ports, container_name,
                                              max_iteration,
    ...
    for i in range(max_iteration):
        ...
        y_new = Running.objective_function(part_x_new, default_values, whether_to_optimize, Ports, container_name)
...
if __name__ == "__main__":
    ...
    default_values = np.array([0.00537113, 0.01956192, 0.09735945, 0.88776312, 0.88714514, 0.57085091,
    ...
    Training_set = np.load('Training_set.npy')[:150, :]
    y_Training_set = np.load('y_Training_set.npy')[:150, :]
    ...
    for j in range(repetition):
        ...
        np.save('.\\Results\\PF-XLO-Reactor-repetition-' + str(j) + '.npy', Pareto_front)
        np.save('.\\Results\\PS-XLO-Reactor-repetition-' + str(j) + '.npy', Pareto_solution)
        with open('.\\Results\\All_PF-XLO-Reactor-repetition-' + str(j) + '.pkl', 'wb') as f:
            pickle.dump(All_PF, f)

        np.save('.\\Results\\RHD-XLO-Reactor-repetition-' + str(j) + '.npy', rhd)
        np.save('.\\Results\\Time-XLO-Reactor-repetition-' + str(j) + '.npy', np.array(t))
