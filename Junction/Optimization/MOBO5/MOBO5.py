...
if __name__ == '__main__':
    ...
    for j in range(repetition):
        ...
        for i in range(initial_size):
            y_initial_points[i, :] = Running.objective_function(num_vars, names, initial_points[i, :],
                                                                    simulation_path, obs_path,
                                                                    start_time, end_time,
                                                                    signal, obs_delta_time, true_flow,
                                                                    true_time, true_speed, true_queue, true_timeLoss,
                                                                    obj)
        ...
        np.save('.\\Results\\PF-MOBO5-Junction-repetition-' + str(j) + '.npy', Pareto_front)
        np.save('.\\Results\\PS-MOBO5-Junction-repetition-' + str(j) + '.npy', Pareto_solution)
        with open('.\\Results\\All_PF-MOBO5-Junction-repetition-' + str(j) + '.pkl', 'wb') as f:
            pickle.dump(All_PF, f)

        np.save('.\\Results\\RHD-MOBO5-Junction-repetition-' + str(j) + '.npy', rhd)
        np.save('.\\Results\\Time-MOBO5-Junction-repetition-' + str(j) + '.npy', np.array(t))
