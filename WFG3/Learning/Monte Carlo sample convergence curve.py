...
def create_ball_image(color='#4A8EBB', size=40):
...
Real_PT = np.load('Real_PT-WFG3.npy')
k = 20
mse_PT = np.zeros(k)
for i in range(k):
    mse_PT[i] = np.sum(np.abs(Real_PT - PT[i, :])) / np.sum(np.abs(Real_PT))

Relative_change = np.zeros(k - 1)
for i in range(k - 1):
    Relative_change[i] = abs(mse_PT[i + 1] - mse_PT[i]) / mse_PT[i]

x_best = 14
y_best = mse_PT[x_best - 1]
...
ax.plot(x, sum_PT, linestyle='-', color='#4A8EBB', linewidth=5, zorder=1)
...
ax.set_ylabel('WMAPE', fontsize=27, fontname='Arial', labelpad=10)
ax.set_xticks(x)
...
ax.text(legend_x + 0.089, legend_y,
        r'$\mathit{\hat{P}_{i}^{\mathrm{total}}}$',
...
plt.tight_layout()
plt.savefig('Monte Carlo WFG3 3Dball.svg',
