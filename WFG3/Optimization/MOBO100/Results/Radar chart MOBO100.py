...
for j in [0, 1, 3, 4, 5, 6, 7, 9]:
    PF.append(np.load('PF-MOBO100-WFG3-repetition-' + str(j) + '.npy'))
    PS.append(np.load('PS-MOBO100-WFG3-repetition-' + str(j) + '.npy'))

all_PF = np.concatenate(PF, axis=0)
all_PF = np.unique(all_PF, axis=0)
all_PF, elite_indices = nsga2_elite_selection(all_PF, 100)
...
for i in range(PS_list.shape[0]):
    line_color = mcolors.to_rgba('#F9B3AD', alpha=0.3)
    marker_color = mcolors.to_rgba('#F9B3AD', alpha=1.0)

    ax.plot(angles, PS_list[i, :], color=line_color, marker='o', markersize=2,
            markerfacecolor=marker_color, markeredgecolor=marker_color, linewidth=0.1, zorder=2)

radius_values = np.linspace(0, 1, num=5)
ax.set_yticks(radius_values)
ax.set_yticklabels([str(value) for value in radius_values],
                   fontdict={'fontsize': 8, 'family': 'Arial'}, zorder=1)
ax.set_xticks(angles[:-1])
ax.set_xticklabels(range(1, len(names) + 1), ha='center', fontsize=10, fontname='Arial')

plt.tight_layout()

plt.savefig('MOBO 100 PS WFG3.svg', dpi=800,
            bbox_inches='tight')
plt.show()
