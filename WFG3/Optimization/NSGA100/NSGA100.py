...
for i in range(repetitions):
    ...
    np.random.seed(20 + i)
    random.seed(30 + i)
    algorithm = NSGA2(pop_size=410, n_offsprings=10, sampling=MySampling(), eliminate_duplicates=True)
    ...
    np.save('.\\Results\\Time-NSGA100-WFG3-repetition-' + str(i) + '.npy', time.time() - start_time)
    ...
    np.save('.\\Results\\PF-NSGA100-WFG3-repetition-' + str(i) + '.npy', Pareto_front)
    np.save('.\\Results\\PS-NSGA100-WFG3-repetition-' + str(i) + '.npy', Pareto_solution)

    np.save('.\\Results\\RHD-NSGA100-WFG3-repetition-' + str(i) + '.npy', np.array(rhd))
