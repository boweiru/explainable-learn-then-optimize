...
def Bayesian_optimization_Simulation_scenario(num_vars, bounds, num_vars_all, Training_set,
                                              y_Training_set, default_values, Model, r,
                                              max_iteration, whether_to_optimize, problem, time_start, seed):
...
if __name__ == "__main__":
    ...
    y_Training_set = y_Training_set[:400, :]
    ...
    for j in range(repetition):
        ...
        Pareto_front, Pareto_solution, All_PF, t = Bayesian_optimization_Simulation_scenario(num_vars, bounds, num_vars_all, Training_set,
                                                  y_Training_set, default_values, Model, r,
                                                  max_iteration, whether_to_optimize, problem, time_start, repetition)
        for i in range(max_iteration + 1):
            rhd.append(Relative_Hypervolume(All_PF[i], r, r_good))
            print(i, rhd[-1])

        np.save('.\\Results\\PF-XLO-WFG3-repetition-' + str(j) + '.npy', Pareto_front)
        np.save('.\\Results\\PS-XLO-WFG3-repetition-' + str(j) + '.npy', Pareto_solution)
        with open('.\\Results\\All_PF-XLO-WFG3-repetition-' + str(j) + '.pkl', 'wb') as f:
            pickle.dump(All_PF, f)

        np.save('.\\Results\\RHD-XLO-WFG3-repetition-' + str(j) + '.npy', rhd)
        np.save('.\\Results\\Time-XLO-WFG3-repetition-' + str(j) + '.npy', np.array(t))
