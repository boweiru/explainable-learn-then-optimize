...
PF_MOBO4 = []
for i in range(10):
    PF_MOBO4.append(np.load('PF-MOBO4-WFG3-repetition-' + str(i) + '.npy'))
all_PF_MOBO4 = np.concatenate(PF_MOBO4, axis=0)
all_PF_MOBO4 = np.unique(all_PF_MOBO4, axis=0)
all_PF_MOBO4, elite_indices = nsga2_elite_selection(all_PF_MOBO4, 100)

PS = []
for j in range(10):
    PS.append(np.load('PS-MOBO4-WFG3-repetition-' + str(j) + '.npy'))
...
plt.savefig('MOBO 4 PS WFG3.svg', dpi=800,
            bbox_inches='tight')
plt.show()