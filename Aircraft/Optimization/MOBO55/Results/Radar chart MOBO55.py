...
PS = []
for j in range(5):
    PS.append(np.load('PS-MOBO55-Aircraft-repetition-' + str(j) + '.npy'))
...
for i in range(PS_list.shape[0]):
    ...
    ax.plot(angles, PS_list[i, :], color=line_color, marker='o', markersize=2,
            markerfacecolor=marker_color, markeredgecolor=marker_color, linewidth=0.6, zorder=2)
...
plt.savefig('MOBO 55 PS Aircraft.svg', dpi=800,
            bbox_inches='tight')
plt.show()
