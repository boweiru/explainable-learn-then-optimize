...
def EIM(x, GP, Pareto_front, x_scaler, y_scalers):
    ...
    for i in range(Pareto_front.shape[1]):
        ...
        y_std[i] = var[0] * y_scalers[i].scale_[0]
...
def Bayesian_optimization_Simulation_scenario(num_vars, bounds, initial_points,
                                              y_initial_points, defaults, whether_to_optimize, r, max_iteration, time_start, repetition):
...
if __name__ == '__main__':
    ...
    for j in range(repetition):
        ...
        np.save('.\\Results\\PF-MOBO55-Aircraft-repetition-' + str(j) + '.npy', Pareto_front)
        np.save('.\\Results\\PS-MOBO55-Aircraft-repetition-' + str(j) + '.npy', Pareto_solution)
        with open('.\\Results\\All_PF-MOBO55-Aircraft-repetition-' + str(j) + '.pkl', 'wb') as f:
            pickle.dump(All_PF, f)

        np.save('.\\Results\\RHD-MOBO55-Aircraft-repetition-' + str(j) + '.npy', rhd)
        np.save('.\\Results\\Time-MOBO55-Aircraft-repetition-' + str(j) + '.npy', np.array(t))
