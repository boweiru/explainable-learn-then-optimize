...
whether_to_optimize[46] = 1
whether_to_optimize[44] = 1
whether_to_optimize[9] = 1

num_vars = np.sum(whether_to_optimize == 1)
names = [r'$x_{8}$', r'$x_{9}$', r'$x_{10}$', r'$x_{45}$', r'$x_{47}$']
bounds = bounds_all[np.where(whether_to_optimize == 1)[0]]

PS = []
for j in range(5):
    PS.append(np.load('PS-MOBO5-Aircraft-repetition-' + str(j) + '.npy'))
...
plt.savefig('MOBO 5 PS Aircraft.svg', dpi=800,
            bbox_inches='tight')
plt.show()
