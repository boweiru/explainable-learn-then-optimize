...
def Bayesian_optimization_Simulation_scenario(num_vars, bounds, whether_to_optimize, num_vars_all, Model,
                                              Training_set,
    ...
    for i in range(max_iteration):
        ...
        y_new = Running.objective_function(part_x_new, default_values, whether_to_optimize)
...
if __name__ == "__main__":
    ...
    Training_set = np.load('Training_set.npy')[:180, :]
    y_Training_set = np.load('y_Training_set.npy')[:180, :]
    ...
    for j in range(repetition):
        ...
        Pareto_front, Pareto_solution, All_PF, t = Bayesian_optimization_Simulation_scenario(num_vars, bounds, whether_to_optimize, num_vars_all, Model,
                                                  Training_set,
        ...
        np.save('.\\Results\\PF-XLO-Aircraft-repetition-' + str(j) + '.npy', Pareto_front)
        np.save('.\\Results\\PS-XLO-Aircraft-repetition-' + str(j) + '.npy', Pareto_solution)
        with open('.\\Results\\All_PF-XLO-Aircraft-repetition-' + str(j) + '.pkl', 'wb') as f:
            pickle.dump(All_PF, f)

        np.save('.\\Results\\RHD-XLO-Aircraft-repetition-' + str(j) + '.npy', rhd)
        np.save('.\\Results\\Time-XLO-Aircraft-repetition-' + str(j) + '.npy', np.array(t))
