...
for dim in dims:
    ...
    WMAPE_MSG.append(compute_total_wmape_from_preds(P_BAG, y_Testing_set))
...
line7, = plt.plot(x, sum_WMAPE_MSG_mean, linestyle='-', color=mcolors.to_rgba('#AC99D2'),
                  label='MSG', linewidth=5, zorder=14)
ax.fill_between(x, sum_WMAPE_MSG_mean - sum_WMAPE_MSG_std,
                sum_WMAPE_MSG_mean + sum_WMAPE_MSG_std, color=mcolors.to_rgba('#AC99D2'), alpha=0.2, zorder=7)
...
plt.tight_layout()
plt.savefig('MSG VS Single Aircraft.svg', dpi=800,
            bbox_inches='tight')
plt.show()
print(ax.get_ylim())
