...
for dim in dims:
    ...
    WMAPE_XGBoost.append(compute_total_wmape_from_preds(P_XGB,  y_Testing_set))
    WMAPE_CatBoost.append(compute_total_wmape_from_preds(P_Cat,  y_Testing_set))
    WMAPE_Model.append(compute_total_wmape_from_preds(P_Mod,     y_Testing_set))
    WMAPE_Model_Data.append(compute_total_wmape_from_preds(P_MDat, y_Testing_set))
...
sum_WMAPE_XGBoost_mean, sum_WMAPE_XGBoost_std = mean_std_per_dim(WMAPE_XGBoost)
sum_WMAPE_CatBoost_mean, sum_WMAPE_CatBoost_std = mean_std_per_dim(WMAPE_CatBoost)
sum_WMAPE_Model_mean, sum_WMAPE_Model_std = mean_std_per_dim(WMAPE_Model)
sum_WMAPE_Model_Data_mean, sum_WMAPE_Model_Data_std = mean_std_per_dim(WMAPE_Model_Data)

fig, ax = plt.subplots(figsize=(9, 8))
x = range(1, 11)

line1, = plt.plot(x, sum_WMAPE_CatBoost_mean, linestyle='-', color=mcolors.to_rgba('#70CDBE'),
                  label='Data-het CatBoost', linewidth=5, zorder=5)
ax.fill_between(x, sum_WMAPE_CatBoost_mean - sum_WMAPE_CatBoost_std,
                   sum_WMAPE_CatBoost_mean + sum_WMAPE_CatBoost_std,
                   color=mcolors.to_rgba('#70CDBE'), alpha=0.2, zorder=1)

line2, = ax.plot(x, sum_WMAPE_XGBoost_mean, linestyle='-', color=mcolors.to_rgba('#AC99D2'),
                 label='Data-het XGBoost', linewidth=5, zorder=8)
ax.fill_between(x, sum_WMAPE_XGBoost_mean - sum_WMAPE_XGBoost_std,
                   sum_WMAPE_XGBoost_mean + sum_WMAPE_XGBoost_std,
                   color=mcolors.to_rgba('#AC99D2'), alpha=0.2, zorder=4)

line3, = plt.plot(x, sum_WMAPE_Model_mean, linestyle='-', color=mcolors.to_rgba('#F8B072'),
                  label='Model-het', linewidth=5, zorder=6)
ax.fill_between(x, sum_WMAPE_Model_mean - sum_WMAPE_Model_std,
                   sum_WMAPE_Model_mean + sum_WMAPE_Model_std,
                   color=mcolors.to_rgba('#F8B072'), alpha=0.2, zorder=2)

line4, = plt.plot(x, sum_WMAPE_Model_Data_mean, linestyle='-', color=mcolors.to_rgba('#4F99C9'),
                  label='Fully-het', linewidth=5, zorder=7)
ax.fill_between(x, sum_WMAPE_Model_Data_mean - sum_WMAPE_Model_Data_std,
                   sum_WMAPE_Model_Data_mean + sum_WMAPE_Model_Data_std,
...
plt.tight_layout()
plt.savefig('MSG comparison Aircraft.svg', dpi=800,
            bbox_inches='tight')
plt.show()
