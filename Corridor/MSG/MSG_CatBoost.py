...
class BaggingMeta(BaseEstimator, RegressorMixin, TransformerMixin):
    def __init__(self, obj, n_estimators=10):
        ...
        self.Meta_scaler = [StandardScaler() for _ in range(self.n_estimators)]
        self.y_scalers = [StandardScaler() for _ in range(self.obj)]

    def fit(self, X, y):
        n = X.shape[0]
        for i in range(self.n_estimators):
            indices = np.random.choice(n, size=int(X.shape[0] * 0.75), replace=True)
