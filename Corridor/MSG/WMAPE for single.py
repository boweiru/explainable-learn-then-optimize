...
# ----------------------------
repetitions = 10
dims = [60 * i for i in range(1, 11)]  # 30..300
y_Testing_set = pd.read_excel('y_Testing_set_60input_noisy.xlsx', header=None).values  # (n_test, obj)
...
def compute_total_wmape_from_preds(preds, y_true):
    """
    preds: (repetitions, n_test, obj)
    y_true: (n_test, obj)
    return: total_wmape (repetitions,)
            每次重复上的总 WMAPE (所有目标合并计算)
    """
    reps, n_test, k = preds.shape
    assert y_true.shape == (n_test, k), f"y_true shape {y_true.shape} 与预测 {preds.shape} 不匹配"
    denom = np.sum(np.abs(y_true))  # 所有目标一起
    wmape = np.zeros(reps)
    for r in range(reps):
        abs_err = np.abs(y_true - preds[r])  # (n_test, obj)
...
for dim in dims:
    P_BAG = np.load(f'PRED-GPSG_XGBoost-{dim}.npy')
...
plt.tight_layout()
# plt.savefig('F:\JinJixia\Paper\Screening-Optimization/Nature\Figure/20250910/20260414' + '\MSG VS Single Songshanhu.svg', dpi=800,
#             bbox_inches='tight')
plt.show()
print(ax.get_ylim())
