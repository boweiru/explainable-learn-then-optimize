...
class MyProblem(ElementwiseProblem):
    def __init__(self):
        super().__init__(n_var=9,
                         ...
                         n_constr=0,
                         xl=np.array(
                             [3.0, 0.2, 0.0, 1.0, 1.0, 1e-09, 0.1, 1e-09, 0.0]),
                         xu=np.array(
                             [7.0, 2.0, 1.0, 10.0, 3.0, 3.0, 10.0, 10.0, 1.0]))
        self.num_vars_all = 9
        self.names_all = []
...
for i in range(repetitions):
    ...
    np.random.seed(20 + i)
    random.seed(30 + i)
    algorithm = NSGA2(pop_size=20, n_offsprings=10, sampling=MySampling(), eliminate_duplicates=True)
    ...
    np.save('.\\Results\\Time-NSGA9-Corridor-repetition-' + str(i) + '.npy', time.time() - start_time)
    ...
    np.save('.\\Results\\PF-NSGA9-Corridor-repetition-' + str(i) + '.npy', Pareto_front)
    np.save('.\\Results\\PS-NSGA9-Corridor-repetition-' + str(i) + '.npy', Pareto_solution)

    np.save('.\\Results\\RHD-NSGA9-Corridor-repetition-' + str(i) + '.npy', np.array(rhd))
