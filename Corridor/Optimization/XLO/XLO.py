...
def Bayesian_optimization_Simulation_scenario(num_vars, names, bounds, num_vars_all, Training_set,
    ...
    for i in range(max_iteration):
        ...
        y_new = Running.objective_function(num_vars, names, part_x_new, simulation_path, obs_path, start_time,
...
if __name__ == "__main__":
    ...
    Training_set = Training_set[:180, :]
    y_Training_set = y_Training_set[:180, :]
    ...
    for j in range(repetition):
        ...
        Pareto_front, Pareto_solution, All_PF, t = Bayesian_optimization_Simulation_scenario(num_vars, names, bounds,
                                                                                             ...
                                                                                             true_queue, time_start, repetition)
        for i in range(max_iteration + 1):
            rhd.append(Relative_Hypervolume(All_PF[i], r, r_good))
            print(i, rhd[-1])

        np.save('.\\Results\\PF-XLO-Corridor-repetition-' + str(j) + '.npy', Pareto_front)
        np.save('.\\Results\\PS-XLO-Corridor-repetition-' + str(j) + '.npy', Pareto_solution)
        with open('.\\Results\\All_PF-XLO-Corridor-repetition-' + str(j) + '.pkl', 'wb') as f:
            pickle.dump(All_PF, f)

        np.save('.\\Results\\RHD-XLO-Corridor-repetition-' + str(j) + '.npy', rhd)
        np.save('.\\Results\\Time-XLO-Corridor-repetition-' + str(j) + '.npy', np.array(t))
